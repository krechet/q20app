<template>
	<div class="wrapper">
		...
		<h3 v-if="$store.state.messages.role === 'owner'" class="note">
			The session word is: {{ $store.state.messages.word }}
		</h3>
		<h3 v-if="!sessionStarted && $store.state.messages.role === 'owner'" class="annotation">
			Player 2 is expected to join the session (ID:{{ session_id }}) from another device to start asking questions
		</h3>
		<QuestionItems
			:items="
				Object.keys($store.state.messages.items)
					.map(k => $store.state.messages.items[k])
					.sort((a, b) =>
		...
		<QuestionForm
			v-if="
				$store.state.messages.role === 'player' &&
					!$store.state.messages.gameStopped
			"
			@submit="submit"
			:waiting="waiting"
		/>
